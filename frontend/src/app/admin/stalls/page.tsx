...
export default function AdminStallsPage() {
  ...
  const actionMutation = useMutation({
    mutationFn: ({ id, action }: { id: string; action: string }) =>
      api.patch(`/admin/stalls/${id}/${action}`).then((r) => r.data),
    onSuccess: (_, vars) => {
                ...
                return (
                  <div key={stall.id} className="px-5 py-4 flex items-center gap-4 hover:bg-gray-50/50 transition-colors">
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      {stall.status !== 'ACTIVE' && (
                        <button
                          onClick={() => actionMutation.mutate({ id: stall.id, action: 'approve' })}
                      ...
                      {stall.status === 'ACTIVE' && (
                        <button
                          onClick={() => actionMutation.mutate({ id: stall.id, action: 'suspend' })}
                          disabled={actionMutation.isPending}
