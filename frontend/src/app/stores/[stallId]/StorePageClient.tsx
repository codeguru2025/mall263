...
export default function StorePageClient() {
  ...
  const followStatusQ = useQuery({
    queryKey: ['stall-follow-status', stallId],
    queryFn: () => api.get(`/api/v1/stalls/${stallId}/follow-status`).then((r) => r.data as { following: boolean }),
  ...
  const followMut = useMutation({
    mutationFn: () => api.post(`/api/v1/stalls/${stallId}/follow`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['stall-follow-status', stallId] }),
  });
  const unfollowMut = useMutation({
    mutationFn: () => api.delete(`/api/v1/stalls/${stallId}/follow`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['stall-follow-status', stallId] }),
  });
  ...
  const hasVirtualWalk = Array.isArray(walkData) && walkData.length > 0;
              ...
              <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                ...
                <div className="flex-1 text-center sm:text-left min-w-0">
                  ...
                  <div className="flex items-center justify-center sm:justify-start gap-3 mt-3 flex-wrap">
                    ...
                    {hasVirtualWalk && (
                      <Link
                        href={`/stores/${stallId}/walk`}
                        className="inline-flex items-center gap-1.5 text-xs font-bold text-white bg-navy-700 hover:bg-navy-800 transition-colors rounded-xl px-3 py-1.5"
                      >
                        <Video className="w-3.5 h-3.5" /> Virtual Walk
                      </Link>
                    )}
                    <button
                      type="button"
                      ...
                      className={`inline-flex items-center gap-1.5 text-xs font-bold rounded-xl px-3 py-1.5 transition-colors disabled:opacity-60 ${
                        isFollowing
                          ? 'text-brand-green bg-green-50 border border-green-200 hover:bg-green-100'
                          : 'text-white bg-brand-orange hover:bg-orange-600'
                      }`}
                    >
                      {isFollowing ? (
