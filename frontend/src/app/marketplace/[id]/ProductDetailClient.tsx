...
export default function ProductDetailClient() {
  ...
  useEffect(() => {
    if (!id) return;
    try {
      const stored = localStorage.getItem('mall263_wishlist');
  ...
  const toggleWishlist = useCallback(() => {
    ...
    try {
      ...
      localStorage.setItem('mall263_wishlist', JSON.stringify(next));
  ...
  // Buyer content gate: after trial ends with no active subscription
  const buyerGated = isAuthenticated && isBuyer && subStatus && !subStatus.fullyAccess;

  const { data: reviewsData, isLoading: reviewsLoading } = useQuery({
    queryKey: ['product-reviews', id],
    queryFn: () => api.get(`/api/v1/reviews/product/${id}`).then((r) => r.data),
    enabled: !!id,
  });
        ...
        <div className="relative w-full aspect-[4/3] sm:aspect-[16/9] overflow-hidden bg-white">
          ...
          <div className="absolute top-0 left-0 right-0 px-4 pt-3 flex items-center gap-3 sm:pt-4">
            ...
            <button
              ...
              className="w-9 h-9 rounded-full bg-white/20 backdrop-blur-md border border-white/30 flex items-center justify-center shadow hover:bg-white/30 transition-colors flex-shrink-0"
          ...
          {buyerGated && (
            <div className="absolute bottom-4 left-4 bg-black/50 backdrop-blur-lg border border-white/20 text-white/70 font-black text-xl px-4 py-1.5 rounded-2xl shadow-lg flex items-center gap-2">
              🔒 Subscribe
            </div>
          )}
        ...
        <div className="px-4 pt-5 pb-28 sm:pb-6 space-y-5">
          ...
          <div className="bg-white rounded-2xl border-2 border-gray-100 p-5 space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-navy-700">Reviews</h3>
              <p className="text-sm text-gray-500">
                {reviewsData?.avgRating ? `${Number(reviewsData.avgRating).toFixed(1)} / 5` : 'No ratings yet'}
              </p>
            </div>
            {reviewsLoading ? (
                    ...
                    <div key={review.id} className="rounded-xl border border-gray-100 p-3">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          ...
                          <p className="text-xs text-gray-500">{'★'.repeat(review.rating)}{'☆'.repeat(5 - review.rating)}</p>
                        </div>
                        {isMine && (
                          <button
                            type="button"
                      ...
                      {review.title ? <p className="text-sm font-semibold text-gray-800 mt-2">{review.title}</p> : null}
            ...
            {isAuthenticated && (
              <div className="rounded-xl bg-gray-50 p-3 space-y-2">
                <p className="text-sm font-semibold text-navy-700">Write a review</p>
                <select
                  value={reviewRating}
                ...
                <textarea
                  ...
                  className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm min-h-20"
                />
                <button
                  type="button"
                ...
                {createReview.isError && (
                  <p className="text-xs text-red-600">Could not submit review. You may have already reviewed this product.</p>
                )}
              </div>
            )}
          </div>

          {/* Buyer subscription gate — shown after trial expires with no active plan */}
          {buyerGated && (
            <div className="bg-orange-50 border-2 border-brand-orange rounded-2xl p-5 text-center space-y-3">
              <div className="text-3xl">🔒</div>
              <p className="font-black text-navy-700">Subscribe to see seller details & pricing</p>
              <p className="text-sm text-gray-600">
                Your free trial has ended. Top up $1/month to see full product info, seller location, and contact details.
              </p>
              <Link
                href="/subscriptions"
                className="inline-flex items-center justify-center gap-2 bg-brand-orange text-white font-bold text-sm px-5 py-2.5 rounded-xl hover:bg-orange-600 transition-colors"
              >
                <Wallet className="w-4 h-4" /> Subscribe for $1/month
              </Link>
            </div>
          )}

          {/* Stall info */}
          {product.stall && !buyerGated && (
